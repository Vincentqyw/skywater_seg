...
def main():
    ...
    parser.add_argument(
        "--hf", action="store_true", help="Download model from HuggingFace (Realcat/skywater_seg)"
    )
    parser.add_argument("--checkpoint", "-c", type=str, help="Path to PyTorch checkpoint (.pth)")
    parser.add_argument("--onnx", type=str, help="Path to ONNX model (no PyTorch needed)")
    ...
    parser.add_argument("--device", type=str, default="cuda", help="Device: cuda, cpu, coreml")
    parser.add_argument("--export-onnx", type=str, default=None, help="Export to ONNX and exit")
    parser.add_argument("--no-overlay", action="store_true", help="Skip visualization overlay")
    parser.add_argument(
        "--crf", action="store_true", help="Apply CRF post-processing (requires pydensecrf)"
    )
    ...
    paths = (
        sorted([str(f) for f in input_path.glob("*") if f.suffix.lower() in exts])
    ...
    for img_path in paths:
        ...
        if not args.no_overlay:
            cv2.imwrite(
                str(output_path / f"{stem}_vis.jpg"), draw_overlay(img_path, result["mask"])
            )
        logger.info(
            f"[OK] {stem}: sky={result['sky_mask'].sum():,}px, water={result['water_mask'].sum():,}px"
        )
